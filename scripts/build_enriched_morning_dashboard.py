...
def _bars(value: object) -> list[dict[str, float | str]]:
    """Keep up to two years of valid, compact OHLCV/EMA chart observations."""
    if not isinstance(value, Sequence) or isinstance(value, (str, bytes)):
        return []
    ...
    for item in value:
        ...
        for source, target in (
            ...
            if number is not None:
                row[target] = int(round(number)) if target == "v" else round(number, 6)
        parsed.append(row)
...
def _edge_view(raw: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "flow": {"status": _text(flow.get("status")), "alerts": _number(flow.get("alert_count")),
                 "coverageStatus": _text(flow.get("coverage_status"), "LEGACY_UNVERIFIED"),
                 "comparableHistorySessions": _number(flow.get("comparable_history_sessions")),
                 "comparisonStatus": _text(flow.get("comparison_status"), "LEGACY_UNVERIFIED"),
                 ...
                 "history": [{"date": _text(_mapping(item).get("market_date")),
                              "comparable": _mapping(item).get("comparison_eligible") is True,
...
def _evaluation_view(value: object) -> dict[str, Any]:
    ...
    for row in raw_rows:
        ...
        if item.get("daily_tracking_eligible") is False:
            continue
        if raw.get("active_model_version") and item.get("model_role", "ACTIVE_THESIS_V3") == "ACTIVE_THESIS_V3" and item.get("model_version") != raw["active_model_version"]:
            continue
        row_view = {
...
def normalize_run(run: Mapping[str, Any]) -> dict[str, Any]:
    ...
    captured = _mapping(enhanced.get("capture_report")).get("results", ())
    if enhanced and captured:
        cutoff = datetime.fromisoformat(str(run.get("cutoff_at", "")).replace("Z", "+00:00"))
        if cutoff.tzinfo is None:
            raise ValueError("enhanced evidence requires a timezone-aware run cutoff")
        for item in captured:
            fetched = _mapping(item).get("fetched_at")
            if fetched:
                observed = datetime.fromisoformat(str(fetched).replace("Z", "+00:00"))
                if observed.tzinfo is None or observed > cutoff:
                    raise ValueError("enhanced evidence was retrieved after the run cutoff")
...
#codex-screener .me-chart .me-close-area{{fill:var(--me-series-1);opacity:.07}}#codex-screener .me-chart .me-volume{{fill:var(--me-series-5);opacity:.24}}#codex-screener .me-chart .me-volume-up{{fill:var(--me-positive);opacity:.28}}#codex-screener .me-chart .me-volume-down{{fill:var(--me-negative);opacity:.28}}#codex-screener .me-chart .me-volume-average{{stroke:var(--me-series-4);stroke-width:1.25;fill:none}}#codex-screener .me-chart .me-sma200{{stroke:var(--me-series-4);stroke-width:1.6;fill:none}}#codex-screener .me-chart .me-range-line{{stroke:var(--me-series-5);stroke-width:1;stroke-dasharray:4 4;fill:none}}#codex-screener .me-chart .me-bb-band{{fill:var(--me-series-4);opacity:.09}}#codex-screener .me-chart .me-bb-line{{stroke:var(--me-series-4);stroke-width:1;stroke-dasharray:4 3;fill:none}}#codex-screener .me-chart .me-latest{{fill:var(--me-series-1);stroke:var(--me-bg);stroke-width:2}}
...
#codex-screener .me-chart .me-analog{{stroke:var(--me-series-1);stroke-width:2;fill:none}}#codex-screener .me-chart .me-analog-band{{fill:var(--me-series-1);opacity:.12;stroke:none}}#codex-screener .me-chart .me-baseline{{stroke:var(--me-muted);stroke-width:1;stroke-dasharray:4 4}}#codex-screener .me-chart .me-zero{{stroke:var(--me-fg);stroke-width:1;opacity:.55}}#codex-screener .me-chart .me-ref-level{{stroke:var(--me-muted);stroke-width:1;opacity:.42}}#codex-screener .me-chart .me-end-connector{{stroke:var(--me-muted);stroke-width:1;opacity:.7}}#codex-screener .me-end-label{{fill:var(--me-fg)!important;font-weight:500}}#codex-screener .me-end-center{{fill:var(--me-warning-fg)!important}}#codex-screener .me-levels{{display:flex;flex-wrap:wrap;align-items:center;gap:5px 13px;margin-top:6px;padding:6px 9px;border-top:1px solid var(--me-border);border-bottom:1px solid var(--me-border);font-size:11px;color:var(--me-muted)}}#codex-screener .me-levels b{{color:var(--me-fg);font-weight:500}}#codex-screener .me-level-key{{color:var(--me-accent);font-weight:500}}#codex-screener .me-pattern-read{{margin:8px 0;padding:8px 10px;background:var(--me-panel-2);font-size:11px;line-height:1.5}}#codex-screener .me-pattern-read b{{font-weight:500}}#codex-screener .me-controls{{justify-content:flex-start}}#codex-screener #me-series{{margin-left:auto}}#codex-screener .me-study-control{{display:flex;align-items:center;gap:5px;color:var(--me-muted);font-size:10px;text-transform:uppercase;letter-spacing:.05em}}#codex-screener .me-study-control[hidden]{{display:none}}#codex-screener .me-study-control select{{max-width:132px;border:0;border-bottom:1px solid var(--me-border);border-radius:0;background:transparent;color:var(--me-fg);padding:3px 1px;font:inherit;font-size:11px;text-transform:none;letter-spacing:0;cursor:pointer}}#codex-screener .me-study-control select:focus-visible{{outline:1px solid var(--me-accent);outline-offset:2px}}
#codex-screener .me-codex-dialog{{width:min(620px,calc(100% - 24px));max-height:calc(100% - 24px);padding:0;border:1px solid var(--me-border);border-radius:0;background:var(--me-panel);color:var(--me-fg);font:inherit}}#codex-screener .me-codex-dialog::backdrop{{background:color-mix(in srgb,var(--me-bg) 78%,transparent)}}#codex-screener .me-codex-dialog form{{display:grid;gap:9px;padding:12px}}#codex-screener .me-codex-dialog-head{{display:flex;align-items:baseline;justify-content:space-between;gap:10px;border-bottom:1px solid var(--me-border);padding-bottom:8px}}#codex-screener .me-codex-dialog-head b{{font-size:12px;font-weight:500}}#codex-screener .me-codex-dialog textarea{{width:100%;min-height:210px;resize:vertical;border:1px solid var(--me-border);border-radius:0;background:var(--me-bg);color:var(--me-fg);padding:9px;font:inherit;font-size:11px;line-height:1.45}}#codex-screener .me-codex-dialog-actions{{display:flex;align-items:center;justify-content:flex-end;gap:8px}}#codex-screener .me-codex-dialog button{{border:1px solid var(--me-border);border-radius:0;background:transparent;color:var(--me-fg);padding:6px 9px;cursor:pointer;font-size:11px}}#codex-screener .me-codex-dialog .me-copy-primary{{color:var(--me-accent);border-color:var(--me-accent)}}#codex-screener .me-codex-status{{min-height:15px;color:var(--me-muted);font-size:10px}}
...
@media(max-width:460px){{#codex-screener .me-header{{padding:12px}}#codex-screener .me-alert{{padding:8px 12px;line-height:1.45}}#codex-screener .me-macro{{display:grid;grid-template-columns:1fr 1fr;gap:8px 12px;padding:10px 12px}}#codex-screener .me-macro span{{min-width:0;overflow-wrap:anywhere}}#codex-screener .me-macro span:last-child{{grid-column:1/-1}}#codex-screener .me-section-head{{padding:10px 12px}}#codex-screener .me-rank-row{{grid-template-columns:28px 54px minmax(92px,1fr) 52px;min-height:44px;padding:7px 10px}}#codex-screener .me-last{{font-size:11px}}#codex-screener .me-focus{{padding:10px}}#codex-screener .me-controls,#codex-screener .me-control-group{{align-items:flex-start;flex-wrap:wrap}}#codex-screener .me-control{{min-height:34px;padding:7px 2px}}#codex-screener .me-study-control{{min-height:34px}}#codex-screener .me-study-control select{{min-height:34px;max-width:118px;font-size:16px}}#codex-screener .me-ask{{min-height:38px}}#codex-screener .me-quote{{display:grid;grid-template-columns:1fr 1fr;gap:6px 10px}}#codex-screener .me-chart-wrap{{height:320px}}#codex-screener .me-thesis,#codex-screener .me-eval-grid{{grid-template-columns:1fr}}#codex-screener .me-eval-item,#codex-screener .me-eval-item:nth-child(2n),#codex-screener .me-eval-item:last-child{{grid-column:auto;border-right:0;border-bottom:1px solid var(--me-border)}}#codex-screener .me-eval-item:last-child{{border-bottom:0}}#codex-screener .me-details summary{{display:block;min-height:44px;padding:11px 12px;overflow-wrap:anywhere}}#codex-screener .me-details summary span{{display:block;margin-top:4px;text-align:left}}#codex-screener .me-detail-body{{padding:0 12px 14px}}#codex-screener th,#codex-screener td{{min-width:92px;padding:7px 6px}}#codex-screener th:first-child,#codex-screener td:first-child{{min-width:112px}}}}
...
<section class="me-stock" aria-label="Selected stock analysis"><div class="me-scope-head"><span class="me-scope-name" id="me-stock-scope">SELECTED STOCK</span><span class="me-muted">ticker-specific evidence</span></div><section class="me-focus" aria-label="Selected ticker analysis">
    ...
    <div class="me-controls"><div class="me-control-group" id="me-views" aria-label="Chart view"></div><div class="me-control-group" id="me-ranges" aria-label="Chart range"></div><label class="me-study-control" id="me-study-control" hidden><span>Study</span><select id="me-study" aria-label="Price chart study"></select></label><div class="me-control-group" id="me-series" aria-label="Chart series"></div></div>
...
<dialog class="me-codex-dialog" id="me-codex-dialog" aria-labelledby="me-codex-dialog-title"><form method="dialog"><div class="me-codex-dialog-head"><b id="me-codex-dialog-title">Codex stress test</b><span class="me-muted">local handoff</span></div><div class="me-codex-status" id="me-codex-status" aria-live="polite"></div><textarea id="me-codex-prompt" readonly aria-label="Codex stress-test prompt"></textarea><div class="me-codex-dialog-actions"><button type="button" class="me-copy-primary" id="me-copy-prompt">Copy prompt</button><button type="submit">Close</button></div></form></dialog>
<script>
(()=>{{
const DATA=__DATA__;
...
const rangeMap={{'1M':22,'3M':66,'6M':126,'1Y':252,'2Y':504,'ALL':null}};
const initialTicker=(()=>{{try{{return new URL(location.href).searchParams.get('ticker')?.toUpperCase()}}catch{{return null}}}})();
let selected=Math.max(0,DATA.entries.findIndex(e=>e.ticker===initialTicker)),range='6M',chartView='HISTORY',chartStudy='NONE',watchView='RANKING',lockedIndex=null,lastWidth=0,productMode=(()=>{{try{{return new URL(location.href).searchParams.get('mode')==='focus'?'FOCUS':'RESEARCH'}}catch{{return 'RESEARCH'}}}})();
const visible={{c:true,e20:true,e50:true}};
const forecastVisible={{v4:true,v3:true,iv:true,challengers:false}};

async function copyCodexPrompt(prompt){{try{{if(window.navigator?.clipboard?.writeText){{await window.navigator.clipboard.writeText(prompt);return true}}}}catch{{}}const field=document.createElement('textarea');field.value=prompt;field.setAttribute('readonly','');field.style.position='fixed';field.style.opacity='0';document.body.appendChild(field);field.select();let copied=false;try{{copied=document.execCommand('copy')}}catch{{}}field.remove();return copied}}
function openCodexHandoff(title,prompt,copied){{const dialog=byId('me-codex-dialog'),field=byId('me-codex-prompt'),status=byId('me-codex-status');byId('me-codex-dialog-title').textContent=title;field.value=prompt;status.textContent=copied?'Prompt copied. Paste it into the current Codex task.':'Automatic copy is unavailable. Select the prompt, copy it, and paste it into the current Codex task.';if(typeof dialog.showModal==='function')dialog.showModal();else dialog.setAttribute('open','');byId('me-copy-prompt').onclick=async()=>{{const success=await copyCodexPrompt(prompt);status.textContent=success?'Prompt copied. Paste it into the current Codex task.':'Select the prompt and copy it manually.';if(!success){{field.focus();field.select()}}}}}}
async function requestCodexStressTest(prompt,title){{if(window.openai?.sendFollowUpMessage){{try{{await window.openai.sendFollowUpMessage({{prompt,title}});return 'SENT'}}catch{{}}}}const copied=await copyCodexPrompt(prompt);openCodexHandoff(title,prompt,copied);return copied?'COPIED':'READY'}}
...
function renderControls(){{
 const views=[['HISTORY','Price history'],['FORECAST','Forecast'],['OUTCOMES','State outcomes']];
 byId('me-views').innerHTML=views.map(([key,label])=>`<button type="button" class="me-control" data-view="${{key}}" aria-pressed="${{key===chartView}}">${{label}}</button>`).join('');
 byId('me-ranges').innerHTML=chartView==='OUTCOMES'?'':Object.keys(rangeMap).map(key=>`<button type="button" class="me-control" data-range="${{key}}" aria-pressed="${{key===range}}">${{key}}</button>`).join('');
 const historySeries=[['c','Close','var(--me-series-1)'],['e20','EMA20','var(--me-series-2)'],['e50','EMA50','var(--me-series-3)']],forecastSeries=[['v4','V4 volatility fan','var(--me-series-4)'],['v3','V3 baseline','var(--me-series-3)'],['iv','IV envelope','var(--me-series-5)'],['challengers','Shadow models','#ff8ad8']],series=chartView==='FORECAST'?forecastSeries:historySeries,state=chartView==='FORECAST'?forecastVisible:visible;
 byId('me-series').innerHTML=chartView==='OUTCOMES'?'':series.map(([key,label,color])=>`<button type="button" class="me-control" data-series="${{key}}" aria-pressed="${{state[key]}}"><span class="me-swatch" style="--swatch:${{color}}"></span>${{label}}</button>`).join('');
 const studyHost=byId('me-study-control'),studySelect=byId('me-study'),hasVolume=DATA.entries[selected].bars.some(row=>n(row.v)),studies=[['NONE','None'],['TREND','Long trend'],['VOLATILITY','Volatility'],...(hasVolume?[['VOLUME','Volume']]:[])];
 studyHost.hidden=chartView!=='HISTORY';if(chartView==='HISTORY'){{if(!studies.some(([key])=>key===chartStudy))chartStudy='NONE';studySelect.innerHTML=studies.map(([key,label])=>`<option value="${{key}}">${{label}}</option>`).join('');studySelect.value=chartStudy;studySelect.onchange=()=>{{chartStudy=studySelect.value;lockedIndex=null;drawActiveChart()}}}}
 byId('me-views').querySelectorAll('button').forEach(button=>button.addEventListener('click',()=>{{chartView=button.dataset.view;if(chartView==='FORECAST')range='1M';lockedIndex=null;renderControls();drawActiveChart()}}));
 byId('me-ranges').querySelectorAll('button').forEach(button=>button.addEventListener('click',()=>{{range=button.dataset.range;lockedIndex=null;renderControls();drawActiveChart()}}));
 byId('me-series').querySelectorAll('button').forEach(button=>button.addEventListener('click',()=>{{const target=chartView==='FORECAST'?forecastVisible:visible;target[button.dataset.series]=!target[button.dataset.series];if(!Object.values(target).some(Boolean))target[Object.keys(target)[0]]=true;lockedIndex=null;renderControls();drawActiveChart()}}))
}}
...
function chartIndicators(rows){{return rows.map((row,index)=>{{const close20=rows.slice(Math.max(0,index-19),index+1).map(item=>item.c).filter(n),close200=rows.slice(Math.max(0,index-199),index+1).map(item=>item.c).filter(n),volume20=rows.slice(Math.max(0,index-19),index+1).map(item=>item.v).filter(n),mean20=close20.length===20?close20.reduce((sum,value)=>sum+value,0)/20:null,variance=n(mean20)?close20.reduce((sum,value)=>sum+(value-mean20)**2,0)/close20.length:null,sd20=n(variance)?Math.sqrt(variance):null,returns=[];for(let i=Math.max(1,index-19);i<=index;i++){{if(n(rows[i].c)&&n(rows[i-1].c)&&rows[i-1].c>0)returns.push(Math.log(rows[i].c/rows[i-1].c))}}const returnMean=returns.length?returns.reduce((sum,value)=>sum+value,0)/returns.length:null,returnVariance=returns.length>1?returns.reduce((sum,value)=>sum+(value-returnMean)**2,0)/(returns.length-1):null,window20=rows.slice(Math.max(0,index-19),index+1),high20=window20.length===20?Math.max(...window20.map(item=>n(item.h)?item.h:item.c).filter(n)):null,low20=window20.length===20?Math.min(...window20.map(item=>n(item.l)?item.l:item.c).filter(n)):null;return {{...row,sma200:close200.length===200?close200.reduce((sum,value)=>sum+value,0)/200:null,bbMid:mean20,bbUpper:n(mean20)&&n(sd20)?mean20+2*sd20:null,bbLower:n(mean20)&&n(sd20)?mean20-2*sd20:null,bbWidth:n(mean20)&&mean20!==0&&n(sd20)?4*sd20/mean20*100:null,rv20:n(returnVariance)?Math.sqrt(returnVariance*252)*100:null,high20,low20,volMean20:volume20.length===20?volume20.reduce((sum,value)=>sum+value,0)/20:null}}}})}
function currentStudyRows(){{const bars=chartIndicators(DATA.entries[selected].bars),count=rangeMap[range]||bars.length;return bars.slice(-Math.min(count,bars.length))}}
function showHistoryQuote(row,previous,first,count){{
 if(!row){{byId('me-quote').innerHTML='<span>No chart observation available.</span>';return}}
 const session=n(previous?.c)&&previous.c!==0?(row.c/previous.c-1)*100:null,period=n(first?.c)&&first.c!==0?(row.c/first.c-1)*100:null,ohl=n(row.o)&&n(row.h)&&n(row.l)?`<span>O / H / L <b>${{money(row.o)}} / ${{money(row.h)}} / ${{money(row.l)}}</b></span>`:'',volume=n(row.v)?`<span>Volume <b>${{compactNum(row.v)}}</b>${{n(row.volMean20)&&row.volMean20>0?' · '+(row.v/row.volMean20).toFixed(1)+'× 20D':''}}</span>`:'',study=chartStudy==='TREND'?`<span>SMA200 <b>${{money(row.sma200)}}</b></span><span>20D range <b>${{money(row.low20)}}–${{money(row.high20)}}</b></span>`:chartStudy==='VOLATILITY'?`<span>Band width <b>${{n(row.bbWidth)?pct(row.bbWidth):'—'}}</b></span><span>20D realized vol <b>${{n(row.rv20)?pct(row.rv20):'—'}}</b></span>`:'';
 byId('me-quote').innerHTML=`<span><b>${{esc(dateLabel(row.d))}}</b></span><span>Close <b>${{money(row.c)}}</b></span><span>Session <b class="${{tone(session)}}">${{pct(session)}}</b></span><span>${{count}} sessions <b class="${{tone(period)}}">${{pct(period)}}</b></span>${{ohl}}${{chartStudy==='VOLUME'?volume:''}}<span>EMA20 <b>${{money(row.e20)}}</b></span><span>EMA50 <b>${{money(row.e50)}}</b></span>${{study}}`;
}}
function drawHistoryChart(){{
 const e=DATA.entries[selected],rows=currentStudyRows(),svg=byId('me-chart'),wrap=byId('me-chart-wrap'),tip=byId('me-tooltip');tip.style.display='none';renderLevels(e);
 if(!rows.length){{svg.innerHTML='<text x="16" y="28">No valid chart data.</text>';showHistoryQuote(null);return}}
 const w=Math.max(320,Math.round(wrap.getBoundingClientRect().width)),h=360,m={{l:w<470?54:62,r:18,t:24,b:43}},pw=w-m.l-m.r,ph=h-m.t-m.b,plotBottom=m.t+ph;
 svg.setAttribute('viewBox',`0 0 ${{w}} ${{h}}`);
 const values=[];rows.forEach(row=>{{if(visible.c){{if(n(row.l))values.push(row.l);if(n(row.h))values.push(row.h);if(!n(row.l)&&n(row.c))values.push(row.c)}}if(visible.e20&&n(row.e20))values.push(row.e20);if(visible.e50&&n(row.e50))values.push(row.e50);if(chartStudy==='TREND'&&n(row.sma200))values.push(row.sma200);if(chartStudy==='VOLATILITY'){{if(n(row.bbUpper))values.push(row.bbUpper);if(n(row.bbLower))values.push(row.bbLower)}}}});if(!values.length)values.push(...rows.map(row=>row.c).filter(n));
 let lo=Math.min(...values),hi=Math.max(...values),pad=(hi-lo||Math.abs(hi)||1)*.075;lo-=pad;hi+=pad;
 const x=index=>m.l+pw*(index/Math.max(1,rows.length-1)),y=value=>m.t+ph*(1-(value-lo)/(hi-lo)),pathFor=key=>{{let started=false;return rows.map((row,index)=>{{if(!n(row[key]))return '';const command=started?'L':'M';started=true;return `${{command}}${{x(index).toFixed(1)}},${{y(row[key]).toFixed(1)}}`}}).join(' ')}};
 const closePoints=rows.map((row,index)=>({{row,index}})).filter(item=>n(item.row.c)),firstPoint=closePoints[0],lastPoint=closePoints.at(-1),closePath=pathFor('c'),areaPath=closePoints.length?`M${{x(firstPoint.index).toFixed(1)}},${{plotBottom}} L${{closePoints.map(item=>`${{x(item.index).toFixed(1)}},${{y(item.row.c).toFixed(1)}}`).join(' L')}} L${{x(lastPoint.index).toFixed(1)}},${{plotBottom}} Z`:'';
 const studyDescription=chartStudy==='TREND'?'SMA200 and the rolling twenty-session range.':chartStudy==='VOLATILITY'?'Twenty-session Bollinger band and annualized realized volatility.':chartStudy==='VOLUME'?'Daily volume with its twenty-session average.':'No additional study.';
 let parts=[`<title>${{esc(e.ticker)}} price history · ${{rows.length}} sessions</title><desc>Daily price history with close, EMA20, EMA50, exact inspection values, period return, and stored positioning references. ${{studyDescription}}</desc><rect class="me-frame" data-chart-frame x="${{m.l}}" y="${{m.t}}" width="${{pw}}" height="${{ph}}"/>`];
 for(let index=0;index<5;index++){{const value=lo+(hi-lo)*index/4,yy=y(value);parts.push(`<line class="me-grid-line" x1="${{m.l}}" x2="${{w-m.r}}" y1="${{yy}}" y2="${{yy}}"/><text x="${{m.l-7}}" y="${{yy+4}}" text-anchor="end">${{money(value)}}</text>`)}}
 const tickCount=w<520?4:5,tickIndexes=Array.from({{length:tickCount}},(_,index)=>Math.round((rows.length-1)*index/(tickCount-1))).filter((value,index,array)=>array.indexOf(value)===index),longRange=rows.length>252,axisDate=value=>{{const date=new Date(value+'T12:00:00Z');return new Intl.DateTimeFormat(undefined,longRange?{{timeZone:'America/New_York',month:'short',year:'2-digit'}}:{{timeZone:'America/New_York',month:'short',day:'numeric'}}).format(date)}};
 tickIndexes.forEach((idx,index)=>{{const anchor=index===0?'start':index===tickIndexes.length-1?'end':'middle';parts.push(`<text x="${{x(idx)}}" y="${{h-18}}" text-anchor="${{anchor}}">${{esc(axisDate(rows[idx].d))}}</text>`)}});parts.push(`<text class="me-axis-title" data-axis="y" x="12" y="14">Price ($)</text><text class="me-axis-title" data-axis="x" x="${{w-m.r}}" y="${{h-4}}" text-anchor="end">Session date</text>`);
 if(chartStudy==='VOLUME'&&rows.some(row=>n(row.v))){{const maxVolume=Math.max(...rows.flatMap(row=>[n(row.v)?row.v:0,n(row.volMean20)?row.volMean20:0])),barWidth=Math.max(.7,Math.min(5,pw/rows.length*.72)),volumeHeight=ph*.18,vy=value=>plotBottom-volumeHeight*value/maxVolume;rows.forEach((row,index)=>{{if(n(row.v)&&maxVolume>0){{const bh=volumeHeight*row.v/maxVolume,up=index===0||!n(rows[index-1].c)||row.c>=rows[index-1].c;parts.push(`<rect class="${{up?'me-volume-up':'me-volume-down'}}" x="${{(x(index)-barWidth/2).toFixed(1)}}" y="${{(plotBottom-bh).toFixed(1)}}" width="${{barWidth.toFixed(1)}}" height="${{bh.toFixed(1)}}"/>`)}}}});let started=false;const averagePath=rows.map((row,index)=>{{if(!n(row.volMean20))return '';const command=started?'L':'M';started=true;return `${{command}}${{x(index).toFixed(1)}},${{vy(row.volMean20).toFixed(1)}}`}}).join(' ');if(averagePath)parts.push(`<path class="me-volume-average" d="${{averagePath}}"/>`)}}
 if(chartStudy==='VOLATILITY'){{const upper=pathFor('bbUpper'),lower=pathFor('bbLower'),bandRows=rows.map((row,index)=>({{row,index}})).filter(item=>n(item.row.bbUpper)&&n(item.row.bbLower));if(bandRows.length){{const band=`M${{bandRows.map(item=>`${{x(item.index).toFixed(1)}},${{y(item.row.bbUpper).toFixed(1)}}`).join(' L')}} L${{bandRows.slice().reverse().map(item=>`${{x(item.index).toFixed(1)}},${{y(item.row.bbLower).toFixed(1)}}`).join(' L')}} Z`;parts.push(`<path class="me-bb-band" d="${{band}}"/><path class="me-bb-line" d="${{upper}}"/><path class="me-bb-line" d="${{lower}}"/>`)}}}}
 if(visible.c&&areaPath)parts.push(`<path class="me-close-area" d="${{areaPath}}"/>`);
 const refs=[e.signals.putWall,e.signals.gammaFlip,e.signals.gammaMagnet,e.signals.callWall].filter(value=>n(value)&&value>=lo&&value<=hi);refs.forEach(value=>{{const yy=y(value);parts.push(`<line class="me-ref-level" x1="${{m.l}}" x2="${{w-m.r}}" y1="${{yy}}" y2="${{yy}}"/>`)}});
 if(visible.c)parts.push(`<path class="me-close" d="${{closePath}}"/>`);if(visible.e20)parts.push(`<path class="me-ema20" d="${{pathFor('e20')}}"/>`);if(visible.e50)parts.push(`<path class="me-ema50" d="${{pathFor('e50')}}"/>`);if(chartStudy==='TREND'){{const sma=pathFor('sma200');if(sma)parts.push(`<path class="me-sma200" d="${{sma}}"/>`);const last=rows.at(-1);if(n(last.high20))parts.push(`<line class="me-range-line" x1="${{m.l}}" x2="${{w-m.r}}" y1="${{y(last.high20)}}" y2="${{y(last.high20)}}"/>`);if(n(last.low20))parts.push(`<line class="me-range-line" x1="${{m.l}}" x2="${{w-m.r}}" y1="${{y(last.low20)}}" y2="${{y(last.low20)}}"/>`)}}
 if(lastPoint&&visible.c)parts.push(`<circle class="me-latest" cx="${{x(lastPoint.index)}}" cy="${{y(lastPoint.row.c)}}" r="4"/>`);
 if(w>=520&&closePoints.length){{const highs=closePoints.map(item=>({{...item,value:n(item.row.h)?item.row.h:item.row.c}})),lows=closePoints.map(item=>({{...item,value:n(item.row.l)?item.row.l:item.row.c}})),high=highs.reduce((a,b)=>a.value>b.value?a:b),low=lows.reduce((a,b)=>a.value<b.value?a:b);[[high,'High',-8],[low,'Low',16]].forEach(([item,label,dy])=>{{const xx=x(item.index),yy=y(item.value),anchor=xx>w*.7?'end':'start',dx=anchor==='end'?-7:7;parts.push(`<circle cx="${{xx}}" cy="${{yy}}" r="2.8" fill="var(--me-series-1)"/><text x="${{xx+dx}}" y="${{yy+dy}}" text-anchor="${{anchor}}">${{label}} ${{money(item.value)}} · ${{esc(axisDate(item.row.d))}}</text>`)}})}}
 parts.push(`<g id="me-hover" style="display:none"><line class="me-guide" id="me-guide" y1="${{m.t}}" y2="${{plotBottom}}"/><circle class="me-marker" id="me-marker-c" r="4" fill="var(--me-series-1)"/><circle class="me-marker" id="me-marker-e20" r="4" fill="var(--me-series-2)"/><circle class="me-marker" id="me-marker-e50" r="4" fill="var(--me-series-3)"/></g><rect class="me-hit" data-chart-hit data-chart-hover-overlay="cross-series" x="${{m.l}}" y="${{m.t}}" width="${{pw}}" height="${{ph}}"/>`);svg.innerHTML=parts.join('');
 const overlay=svg.querySelector('[data-chart-hit]'),hover=svg.querySelector('#me-hover'),guide=svg.querySelector('#me-guide'),inspect=index=>{{const idx=Math.max(0,Math.min(rows.length-1,index)),row=rows[idx],xx=x(idx);hover.style.display='';guide.setAttribute('x1',xx);guide.setAttribute('x2',xx);[['c','me-marker-c'],['e20','me-marker-e20'],['e50','me-marker-e50']].forEach(([key,id])=>{{const marker=svg.querySelector('#'+id);if(visible[key]&&n(row[key])){{marker.style.display='';marker.setAttribute('cx',xx);marker.setAttribute('cy',y(row[key]))}}else marker.style.display='none'}});showHistoryQuote(row,rows[idx-1],rows[0],rows.length);const ohl=n(row.o)&&n(row.h)&&n(row.l)?`<br>O/H/L ${{money(row.o)}} / ${{money(row.h)}} / ${{money(row.l)}}`:'',volume=chartStudy==='VOLUME'&&n(row.v)?`<br>Volume ${{compactNum(row.v)}}${{n(row.volMean20)&&row.volMean20>0?' · '+(row.v/row.volMean20).toFixed(1)+'× 20D':''}}`:'',study=chartStudy==='TREND'?`<br>SMA200 ${{money(row.sma200)}} · 20D ${{money(row.low20)}}–${{money(row.high20)}}`:chartStudy==='VOLATILITY'?`<br>Band width ${{n(row.bbWidth)?pct(row.bbWidth):'—'}} · RV20 ${{n(row.rv20)?pct(row.rv20):'—'}}`:'';tip.innerHTML=`<b>${{esc(dateLabel(row.d))}}</b><br>Close ${{money(row.c)}}${{ohl}}${{volume}}${{visible.e20?'<br>EMA20 '+money(row.e20):''}}${{visible.e50?'<br>EMA50 '+money(row.e50):''}}${{study}}`;tip.style.display='block';tip.style.left=Math.max(4,Math.min(w-230,xx+10))+'px';tip.style.top=Math.max(4,y(row.c)-104)+'px';return idx}};
 overlay.addEventListener('pointermove',event=>{{const rect=svg.getBoundingClientRect(),local=(event.clientX-rect.left)*(w/rect.width),idx=Math.round((local-m.l)/pw*(rows.length-1));if(lockedIndex===null)inspect(idx)}});overlay.addEventListener('pointerdown',event=>{{const rect=svg.getBoundingClientRect(),local=(event.clientX-rect.left)*(w/rect.width),idx=Math.round((local-m.l)/pw*(rows.length-1));lockedIndex=lockedIndex===Math.max(0,Math.min(rows.length-1,idx))?null:inspect(idx);if(lockedIndex===null){{hover.style.display='none';tip.style.display='none';showHistoryQuote(rows.at(-1),rows.at(-2),rows[0],rows.length)}}}});overlay.addEventListener('pointerleave',()=>{{if(lockedIndex===null){{hover.style.display='none';tip.style.display='none';showHistoryQuote(rows.at(-1),rows.at(-2),rows[0],rows.length)}}}});if(lockedIndex!==null)inspect(Math.min(lockedIndex,rows.length-1));else showHistoryQuote(rows.at(-1),rows.at(-2),rows[0],rows.length);lastWidth=w;
}}
function drawActiveChart(){{if(chartView==='OUTCOMES')drawOutcomeChart();else if(chartView==='FORECAST')drawPredictionChart();else drawHistoryChart()}}
...
function flowAlert(e){{const f=e.edge.flow,w=e.whale.flow,deltaSign=n(w.delta)?Math.sign(w.delta):0,premiumSign=n(f.directionalPremium)?Math.sign(f.directionalPremium):0,agreement=deltaSign&&premiumSign&&deltaSign===premiumSign,disagreement=deltaSign&&premiumSign&&deltaSign!==premiumSign,sign=disagreement?0:(deltaSign||premiumSign),history=n(f.comparableHistorySessions)?f.comparableHistorySessions:0,ranked=f.comparisonStatus==='COMPARABLE'&&history>=20&&n(f.percentile),qualified=ranked&&f.percentile>=.9,side=sign>0?'POSITIVE':sign<0?'NEGATIVE':'MIXED',label=qualified?`UNUSUAL ${{side}} FLOW`:ranked?`${{side}} FLOW · NORMAL RANGE`:`${{side}} FLOW · COVERAGE UNVERIFIED`,cls=side==='POSITIVE'?'me-flow-positive':side==='NEGATIVE'?'me-flow-hot':'',agreementText=agreement?'Greek delta agrees':disagreement?'Greek delta conflicts':'Greek delta is inconclusive',percentile=ranked?`${{Math.round(f.percentile*100)}}th magnitude percentile`:'magnitude rank unavailable',read=`${{signedCompact(f.directionalPremium)}} directional premium · ${{percentile}} · ${{history}} comparable sessions · ${{agreementText}}`;return {{label,cls,read,qualified,history}}}}
